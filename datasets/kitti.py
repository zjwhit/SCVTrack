...
class kittiDataset(base_dataset.BaseDataset):
    ...
    def get_num_frames_total(self):
        return sum(self.tracklet_len_list) # note: sum here, not len
    ...
    def _build_tracklet_anno(self):
        ...
        for scene in self.scene_list:
            ...
            df.insert(loc=0, column="scene", value=scene) # add a colume about scene id
            for track_id in df.track_id.unique():
                df_tracklet = df[df["track_id"] == track_id] # select by the track id
