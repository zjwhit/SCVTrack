...
def get_negative(PCs, box, offset=0, scale=1.0, normalize=False):
    if len(PCs) == 0:
        return PointCloud(np.ones((3, 0)))
    new_memory_points = []
    memory = PCs[0]

    # new_points.append(PCs[1].points)
    memory_points = memory.points
    new_memory_points.append(memory_points)
    prev_pc, prev_box = cropAndCenterPC(PCs[1], box, offset=offset, scale=scale, normalize=normalize)

    in_box_mask = nuscenes.utils.geometry_utils.points_in_box(prev_box, prev_pc.points, wlh_factor=1.0)
    # in_box_mask = get_in_box_mask(pc, box)
    in_box_pc = PointCloud(prev_pc.points[:, in_box_mask])

    rand_trans = np.random.uniform(low=-1, high=1, size=3)
    rand_rot = np.random.uniform(low=-45, high=45)
    flip_x, flip_y = np.random.choice([True, False], size=2, replace=True)

    new_in_box_pc, new_box = apply_transform(in_box_pc, prev_box, rand_trans, rand_rot, flip_x, flip_y)

    new_pc = copy.deepcopy(prev_pc)
    new_pc.points[:, in_box_mask] = new_in_box_pc.points

    new_memory_points.append(new_pc.points)
    memory_PC = PointCloud(np.concatenate(new_memory_points, axis=1))
    regularized_points, _ = regularize_pc(memory_PC.points.T, 1024)
    memory_PC = PointCloud(regularized_points.T)
    return memory_PC


def getModel(PCs, box, offset=0, scale=1.0, normalize=False):
    ...
    voxel_size = 0.3
    w, l, h = prev_box.wlh
    shape = np.ceil(np.array([l,w,h])/voxel_size).astype(int)
    p_nums = np.zeros(shape)
    for point in memory_points.T:
        index = np.floor((point + np.array([l/2,w/2,h/2])) / voxel_size).astype(int)
        p_nums[index[0]][index[1]][index[2]] = p_nums[index[0]][index[1]][index[2]] + 1
    in_box_mask = []
    cnt = 0
    for point in prev_pc.points.T:
        index = np.floor((point + np.array([l/2,w/2,h/2])) / voxel_size).astype(int)
        if p_nums[index[0]][index[1]][index[2]] <= 10:
            in_box_mask.append(cnt)
        cnt = cnt + 1


    new_memory_points.append(prev_pc.points[:, in_box_mask])
    memory_PC = PointCloud(np.concatenate(new_memory_points, axis=1))
    return memory_PC


def memory_input(memory_pc, prev_pc, box, offset=0, scale=1.0, normalize=False):
    input_memory_points = []
    prev_pc, prev_box = cropAndCenterPC(prev_pc, box, offset=offset, scale=scale, normalize=normalize)
    voxel_size = 0.3
    w, l, h = prev_box.wlh
    shape = np.ceil(np.array([l,w,h])/voxel_size).astype(int)
    p_nums = np.zeros(shape)
    ...
    in_box_mask = []
    cnt = 0
    for point in memory_pc.points.T:
        index = np.floor((point + np.array([l/2,w/2,h/2])) / voxel_size).astype(int)
        if p_nums[index[0]][index[1]][index[2]] <= 10:
            in_box_mask.append(cnt)
        cnt = cnt + 1

    input_memory_points.append(memory_pc.points[:, in_box_mask])
    memory_PC = PointCloud(np.concatenate(input_memory_points, axis=1))
    return memory_PC
