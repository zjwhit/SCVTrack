...
def motion_processing(data, track_id, prev_frame_id, config, template_transform=None, search_transform=None):
    """
    ...
    memory_directory = "./tmp_memory/" + config.category_name + "/" + "track_id_" + str(track_id) + "_frame_id_"
    negative_directory = "./tmp_memory/negative/" + config.category_name + "/" + "track_id_" + str(track_id) + "_frame_id_"
    memory_id = max(prev_frame_id, 0)
    
    if os.path.exists(memory_directory + str(memory_id) + '.npy'):
        memory_points = np.load(memory_directory + str(memory_id) + '.npy', allow_pickle=False)
        memory = PointCloud(memory_points)
        memory = points_utils.getModel([memory, prev_pc], prev_box)
        if not os.path.exists(memory_directory + str(prev_frame_id) + '.npy'):
            np.save(memory_directory + str(prev_frame_id) + '.npy', memory.points)
    
    else:
        if os.path.exists(memory_directory + '0.npy'):
            memory_points = np.load(memory_directory + '0.npy', allow_pickle=False)
            memory = PointCloud(memory_points)
        else:
            memory, _ = points_utils.cropAndCenterPC(frames[0]['pc'], frames[0]['3d_bbox'])
            np.save(memory_directory + '0.npy', memory.points)
        for i in range(1, len(frames)):
            if not os.path.exists(memory_directory + str(i) + '.npy'):
                try:
                    memory_points = np.load(memory_directory + str(i-1) + '.npy', allow_pickle=False)
                except:
                    time.sleep(1)
                    memory_points = np.load(memory_directory + str(i-1) + '.npy', allow_pickle=False)
                memory = PointCloud(memory_points)
                memory = points_utils.getModel([memory, frames[i]['pc']], frames[i]['3d_bbox'])
                np.save(memory_directory + str(i) + '.npy', memory.points)
        memory = points_utils.getModel([memory, prev_pc], prev_box)
        if not os.path.exists(memory_directory + str(prev_frame_id) + '.npy'):
            np.save(memory_directory + str(prev_frame_id) + '.npy', memory.points)
    ...
    # search_area_box = points_utils.generate_search_area(ref_box, scale=config.bb_scale, offset=config.bb_offset)
    # search_area = np.append(search_area_box.center, search_area_box.wlh).astype('float32')
    ...
    memory_points, _ = points_utils.regularize_pc(add_memory.points.T, config.point_sample_size)
    ...
    # seg_mask_this = np.full((config.point_sample_size), fill_value=0.5)
    ...
    # timestamp_memory = np.full((config.point_sample_size, 1), fill_value=(prev_frame_id-1)/10)
    ...
    data_dict = {
        ...
        'memory_points': memory_points.astype('float32'),
        'box_label': box_label,
        ...
        'ref_box_size': wlh.astype('float32')
    }
    ...
    if getattr(config, 'box_aware', False):
        ...
        memory_bc = points_utils.get_point_to_box_distance(memory_points, ref_box)
        ...
        data_dict.update({'prev_bc': prev_bc.astype('float32'),
                          ...
                          'memory_bc': memory_bc.astype('float32')})
    return data_dict
        ...
        try:
            for i in range(0, self.dataset.get_num_tracklets()):
                if self.tracklet_start_ids[i] <= anno_id < self.tracklet_start_ids[i + 1]:
                    ...
                    # frame_ids = (0, prev_frame_id, this_frame_id)
                    frame_ids = [0]
                    for j in range(1, prev_frame_id):
                        frame_ids.append(j)
                    frame_ids.append(prev_frame_id)
                    frame_ids.append(this_frame_id)
            # first_frame, prev_frame, this_frame = self.dataset.get_frames(tracklet_id, frame_ids=frame_ids)
            frames = self.dataset.get_frames(tracklet_id, frame_ids = frame_ids)
            # every contains pc:point cloud and bb:bbox
            data = {
                # "first_frame": first_frame,
                # "prev_frame": prev_frame,
                # "this_frame": this_frame,
                "pre_frames": frames[0: -2],
                "prev_frame": frames[-2],
                "this_frame": frames[-1],
                "candidate_id": candidate_id}
            return self.processing(data, tracklet_id, prev_frame_id,
                                   self.config,
