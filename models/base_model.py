...
class BaseModel(pl.LightningModule):
    ...
    def build_input_dict(self, sequence, frame_id, results_bbs, memory, **kwargs):
        raise NotImplementedError
    ...
    def evaluate_one_sample(self, data_dict, ref_box):
        ...
        timelist = []
        timelist.append(end_points['segtime'])
        timelist.append(end_points['refinetime'])
        timelist.append(end_points['featuretime'])
        timelist.append(end_points['convtime'])
        timelist.append(end_points['firsttime'])
        timelist.append(end_points['secondtime'])
    

        # refine_box = end_points['estimation_boxes_prev']
        # refine_box_cpu = refine_box.squeeze(0).detach().cpu().numpy()
        # if len(refine_box.shape) == 3:
        #     refine_box_idx = refine_box_cpu[:, 4].argmax()
        #     refine_box_cpu = refine_box_cpu[refine_box_idx, 0:4]
        
        # refined_bb = points_utils.getOffsetBB(last_bb, refine_box_cpu, degrees=self.config.degrees,
        #                                       use_z=self.config.use_z,
        #                                       limit_box=self.config.limit_box)
        return candidate_box, timelist
    ...
    def evaluate_one_sequence(self, sequence):
        ...
        results_bbs = []
        test_bbs = []
        memory, _ = points_utils.cropAndCenterPC(sequence[0]['pc'], sequence[0]['3d_bbox'])
        for frame_id in range(len(sequence)):  # tracklet
            ...
            if frame_id == 0:
                ...
                # test_bbs.append(this_bb)
            # elif frame_id == 1:
            #     # construct input dict
            #     data_dict, ref_bb, memory = self.build_input_dict(sequence, frame_id, results_bbs, memory)
            #     # run the tracker
            #     candidate_box, _ = self.evaluate_one_sample(data_dict, ref_box=ref_bb, last_bb=results_bbs[-1])
            #     results_bbs.append(candidate_box)
            #     continue

            else:
                # construct input dict
                data_dict, ref_bb, memory, num_label = self.build_input_dict(sequence, frame_id, results_bbs, memory)
                # run the tracker
                candidate_box, timelist = self.evaluate_one_sample(data_dict, ref_box=ref_bb)
                segtimes.append(timelist[0])
                refinetimes.append(timelist[1])
                featuretimes.append(timelist[2])
                convtimes.append(timelist[3])
                firsttimes.append(timelist[4])
                secondtimes.append(timelist[5])
                results_bbs.append(candidate_box)
                # test_bbs.append(refine_box)
                # this_bb = sequence[frame_id-1]["3d_bbox"]

            # this_overlap = estimateOverlap(this_bb, test_bbs[-1], dim=self.config.IoU_space,
            #                                up_axis=self.config.up_axis)
            # this_accuracy = estimateAccuracy(this_bb, test_bbs[-1], dim=self.config.IoU_space,
            #                                  up_axis=self.config.up_axis)
    ...
    def validation_epoch_end(self, outputs):
        print('success:' + str(self.success.compute()) + " precision:" + str(self.prec.compute()))
...
class MotionBaseModel(BaseModel):
    ...
    def build_input_dict(self, sequence, frame_id, results_bbs, memory):
        assert frame_id > 0, "no need to construct an input_dict at frame 0"

        # first_frame = sequence[0]
        prev_frame_id = len(sequence)
        prev_frame = sequence[frame_id - 1]
        this_frame = sequence[frame_id]
        # first_pc = first_frame['pc']
        # first_box = results_bbs[0]
        prev_pc = prev_frame['pc']
        this_pc = this_frame['pc']
        ref_box = results_bbs[-1]
        prev_box = prev_frame['3d_bbox']

        wlh = ref_box.wlh * 1.25 + [4.0, 4.0, 4.0]
        # prev_pc, _ = points_utils.getModel([memory, prev_pc], ref_box)
        # _, memory = points_utils.getModel([memory, prev_pc], prev_box)
        memory = points_utils.getModel([memory, prev_pc], ref_box)

        # add_memory = points_utils.memory_input(memory, prev_pc, ref_box)
        add_memory = copy.deepcopy(memory)
        # rot_mat = ref_box.rotation_matrix
        # trans = ref_box.center
        # add_memory.rotate((rot_mat))
        # add_memory.translate(trans)
        # ---------
        ...
        num_points_in_prev_box = geometry_utils.points_in_box(this_frame['3d_bbox'], this_pc.points).sum()
        if num_points_in_prev_box >= 100: # and num_points_in_prev_box < 150:
            num_label = True
        else:
            num_label = False

        memory_points, _ = points_utils.regularize_pc(add_memory.points.T,
        ...
        # seg_mask_this = np.full((self.config.point_sample_size), fill_value=0.5)
        ...
        # timestamp_memory = np.full((self.config.point_sample_size, 1), fill_value=(prev_frame_id-1)/10)
        ...
        # memory_points = np.concatenate([memory_points, timestamp_memory], axis=-1)
        ...
        data_dict = {"points": torch.tensor(stack_points[None, :], device=self.device, dtype=torch.float32),
                     "memory_points": torch.tensor(memory_points[None, :], device=self.device, dtype=torch.float32),
                     "ref_box_size": torch.tensor(wlh[None, :], device=self.device, dtype=torch.float32)}
        if getattr(self.config, 'box_aware', False):
            ...
            memory_bc = points_utils.get_point_to_box_distance(memory_points, canonical_box)
            data_dict.update({'candidate_bc': points_utils.np_to_torch_tensor(candidate_bc.astype('float32'),
                                                                              device=self.device),
                              'memory_bc': points_utils.np_to_torch_tensor(memory_bc.astype('float32'),
                                                                           device=self.device)})
        return data_dict, results_bbs[-1], memory, num_label
        # return data_dict, prev_frame["3d_bbox"], memory
